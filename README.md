...
A lightweight, self‑hosted cloud storage solution built with Spring Boot.  
It supports secure authentication, file upload/download (up to 500 MB per file), and a minimal dashboard – ideal for developers who need local storage while building Java web applications.
...
CloudBucket is a Spring Boot application that stores files in a local directory and uses an in‑memory H2 database by default, making it quick to start in development environments. It ships with:
...
- An extensible architecture that lets you swap storage or authentication modules with minimal effort.
...
| **Database** | In‑memory H2 in `dev` profile; can be replaced with any JDBC database. |
| **Extensibility** | Custom storage or authentication providers can be plugged in. |
...
> The H2 console is enabled only in the `dev` profile. Activate it with `-Dspring.profiles.active=dev`.

---

## Configuration

Edit `src/main/resources/application.properties` or override defaults with environment variables:
...
| `file.upload-dir` | Directory to store files | `/var/cloudbucket/uploads` |
| `server.port` | HTTP port | `8080` |

Environment variables that map to the properties above:
...
2. Run the existing tests with `./mvnw test`.
